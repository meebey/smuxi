    ...
    public class TwitterProtocolManager : ProtocolManagerBase
    {
        ...
        public override void Connect(FrontendManager fm, ServerModel server)
        {
            ...
            try {
                ...
                fm.SetStatus(msg);
                msg     = _("Connection failed! Reason: ");
                builder = CreateMessageBuilder().
                            AppendEventPrefix() .
                            AppendText(msg) .
        ...
        public void CommandSay(CommandModel cmd)
        {
            ...
            if (cmd.Chat.ChatType == ChatType.Group) {
                ...
                switch (twitterChatType) {
                    ...
                    case TwitterChatType.Replies:
                        try {
                            ...
                            var builder = CreateMessageBuilder() .
								AppendEventPrefix() .
								AppendText(_("Could not update" +
								" status - Reason: {0}"), ex.Message);
                            fm.AddMessageToChat(cmd.Chat, builder.ToMessage());
                        }
                        break;
                    case TwitterChatType.DirectMessages:
                        var builder = CreateMessageBuilder() .
							AppendEventPrefix() .
							AppendText(_("Cannot send message" +
                             " - no target specified. " +
                ...
                try {
                    ...
                    var builder = CreateMessageBuilder() .
							AppendEventPrefix() .
							AppendText(_("Could not send message" + 
								" - Reason: {0}"), ex.Message);
                    fm.AddMessageToChat(cmd.Chat, builder.ToMessage());
                }
            } else {
